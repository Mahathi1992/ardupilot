...
static void mavlink_test_power_12v(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_POWER_12V >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_power_12v_t packet_in = {
        17.0,45.0,73.0,41,108
    };
    mavlink_power_12v_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.Roll = packet_in.Roll;
        packet1.Pitch = packet_in.Pitch;
        packet1.Yaw = packet_in.Yaw;
        packet1.Power12_On_Off = packet_in.Power12_On_Off;
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_POWER_12V_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_POWER_12V_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_power_12v_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_power_12v_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_power_12v_pack(system_id, component_id, &msg , packet1.Power12_On_Off , packet1.Roll , packet1.Pitch , packet1.Yaw , packet1.Mount_Mode );
    mavlink_msg_power_12v_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_power_12v_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.Power12_On_Off , packet1.Roll , packet1.Pitch , packet1.Yaw , packet1.Mount_Mode );
    mavlink_msg_power_12v_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_power_12v_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_power_12v_send(MAVLINK_COMM_1 , packet1.Power12_On_Off , packet1.Roll , packet1.Pitch , packet1.Yaw , packet1.Mount_Mode );
    mavlink_msg_power_12v_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

#ifdef MAVLINK_HAVE_GET_MESSAGE_INFO
    MAVLINK_ASSERT(mavlink_get_message_info_by_name("POWER_12V") != NULL);
    MAVLINK_ASSERT(mavlink_get_message_info_by_id(MAVLINK_MSG_ID_POWER_12V) != NULL);
#endif
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_power_12v(system_id, component_id, last_msg);
