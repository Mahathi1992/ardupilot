...
void AP_Mount_Alexmos::init()
{
    ...
    gcs().send_text(MAV_SEVERITY_INFO, "BCG Alexmos INIT" );
    AP_HAL::UARTDriver* alexmos2 = AP::serialmanager().get_alexmos2_uart();
    if (alexmos2 != nullptr){
        alexmos2->begin(AP_SERIALMANAGER_ALEXMOS_BAUD,AP_SERIALMANAGER_ALEXMOS_BUFSIZE_RX,AP_SERIALMANAGER_ALEXMOS_BUFSIZE_TX);
        _port = alexmos2;
        get_boardinfo();
    }
}
...
void AP_Mount_Alexmos::parse_body()
{
    switch (_command_id ) {
        case CMD_BOARD_INFO:
            ...
            gcs().send_text(MAV_SEVERITY_INFO, "BCG V:%i F:%fb%i G3X:%i",_board_version,_current_firmware_version , _firmware_beta_version,_gimbal_3axis );
            break;
