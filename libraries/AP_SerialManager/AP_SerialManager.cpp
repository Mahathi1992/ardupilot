...
extern AP_HAL::UARTDriver *uartTracker, *uart1Debug, *uartDynamixel;
...
void AP_SerialManager::init()
{
    ...
    for (uint8_t i=1; i<SERIALMANAGER_NUM_PORTS; i++) {
	  ...
	  auto *uart = state[i].uart;
        
	  /*
	  if (uart == nullptr){
	      auto *uart2 = state[2].uart;
	      if (uart2 != nullptr){
		  uart2->printf("Fail: %i.\r\n", i);
	      }
	  }
	  */
		    ...
		    /*
                    ...
                    */
                    ...
                    break;
                case SerialProtocol_Dynamixel_MX:
                        //state[i].baud = AP_SERIALMANAGER_DYNAMYIXEL_MX_BAUD;
                        uart->begin(state[i].baud, AP_SERIALMANAGER_DYNAMYIXEL_MX_BUFSIZE_RX, AP_SERIALMANAGER_DYNAMYIXEL_MX_BUFSIZE_TX);
                        //uart->begin(1000000);
                        uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
                        uart->printf("UXHEHD\r\n");
			uartDynamixel = uart;
                ...
                case SerialProtocol_SightLine:
                        //state[i].baud = AP_SERIALMANAGER_SIGHTLINE_BAUD;
                        uart->begin(map_baudrate(state[i].baud), AP_SERIALMANAGER_SIGHTLINE_BUFSIZE_RX, AP_SERIALMANAGER_SIGHTLINE_BUFSIZE_TX);
                        uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
                        uart->printf("2NPF57\r\n");
			uartTracker = uart;
                    break;
                case SerialProtocol_ZASDebug:
                        //state[i].baud = AP_SERIALMANAGER_ZASDEBUG_BAUD;
                        uart->begin(map_baudrate(state[i].baud), AP_SERIALMANAGER_ZASDEBUG_BUFSIZE_RX, AP_SERIALMANAGER_ZASDEBUG_BUFSIZE_TX);
                        uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
                        uart->printf("2PYX57\r\n");
			uart1Debug = uart;
                    break;
                case SerialProtocol_Payload2:
                        //state[i].baud = AP_SERIALMANAGER_PAYLOAD2_BAUD;
                        uart->begin(map_baudrate(state[i].baud), AP_SERIALMANAGER_PAYLOAD2_BUFSIZE_RX, AP_SERIALMANAGER_PAYLOAD2_BUFSIZE_TX);
                        uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
                        uart->printf("FTER62\r\n");
                        payload2 = uart;
                    break;
                case SerialProtocol_AlexmosV2:
                        alexmos2 = uart;
                        uart->begin(map_baudrate(state[i].baud), AP_SERIALMANAGER_ALEXMOS_BUFSIZE_RX, AP_SERIALMANAGER_ALEXMOS_BUFSIZE_TX);
                        uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
                        uart->printf("ALEXV2\r\n");
                        break;
                ...
                case SerialProtocol_EOCamera:
                    state[i].uart->begin(map_baudrate(state[i].baud), 128, 128);
                    state[i].uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
                    break;
                case SerialProtocol_IRCamera:
                    state[i].uart->begin(map_baudrate(state[i].baud), 128, 128);
                    state[i].uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
                    break;
...
AP_HAL::UARTDriver* AP_SerialManager::get_payload2_uart(){
    return payload2;
}

AP_HAL::UARTDriver* AP_SerialManager::get_alexmos2_uart(){
    return alexmos2;
}
