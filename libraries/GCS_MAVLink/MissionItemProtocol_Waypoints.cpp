...
static const GCS_MAVLINK *_downlink = nullptr;
...
MAV_MISSION_RESULT MissionItemProtocol_Waypoints::append_item(const mavlink_mission_item_int_t &mission_item_int)
{
    ...
    if (res != MAV_MISSION_ACCEPTED) {
        if (_downlink != nullptr){
            _downlink->send_text(MAV_SEVERITY_INFO, "Mission Append not accepted.");
        }
    ...
    if (cmd.id == MAV_CMD_DO_JUMP) {
        ...
        if (_downlink != nullptr){
            _downlink->send_text(MAV_SEVERITY_INFO, "Jump Target incorrect.");
        }
    ...
    if (!mission.add_cmd(cmd)) {
        if (_downlink != nullptr){
            _downlink->send_text(MAV_SEVERITY_INFO, "Mission Append add error.");
        }
        return MAV_MISSION_ERROR;
    }
...
MAV_MISSION_RESULT MissionItemProtocol_Waypoints::replace_item(const mavlink_mission_item_int_t &mission_item_int)
{
    ...
    if (!mission.replace_cmd(cmd.index, cmd)) {
            if (_downlink != nullptr){
                _downlink->send_text(MAV_SEVERITY_INFO, "Mission Error.");
            }
        return MAV_MISSION_ERROR;
    }
    return MAV_MISSION_ACCEPTED;
