    ...
    const mavlink_message_t &msg)
{
    ...
    mipValidator.reset();
    mipValidator.setMissionWaypoints(packet.count);
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Num items for Mission Plan: %i", mipValidator.wpCount);
...
void MissionItemProtocol::handle_mission_item(const mavlink_message_t &msg, const mavlink_mission_item_int_t &cmd)
{
    ...
    if (result != MAV_MISSION_ACCEPTED) {
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "Mission Upload Failed. Num Uploaded: %i", mipValidator.numWpReceived);
    ...
    if (request_i > request_last) {
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "Mission Upload Success. Uploaded: %i, Expected: %i, Completion: %s", mipValidator.numWpReceived, mipValidator.wpCount, mipValidator.getCompletionStatus()?"true":"false");
