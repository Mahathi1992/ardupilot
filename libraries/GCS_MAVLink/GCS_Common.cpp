...
To hold the camera Mode value - EO or IR*/
void GCS_MAVLINK::set_camera_mode(uint8_t camera_mode_id)
{
    camera_mode = camera_mode_id;
}
...
/* Added by Akash - To control camera Zoom 
Call the Send status method based on the Camera Mode - EO 0r IR*/
void GCS_MAVLINK::send_camera_settings_status()const
{
    /*
    if(camera_mode == 1){
        AP_EOCamera_Control *eocamera_control = AP::eocamera_control();
        if (eocamera_control == nullptr) {
           // gcs().send_text(MAV_SEVERITY_CRITICAL, "Inside EO");
                return;
        }
       //gcs().send_text(MAV_SEVERITY_CRITICAL, "Inside EO1");
        eocamera_control->send_camera_settings_status(chan);
    }
    else if(camera_mode == 2){
        AP_IRCamera_Control *ircamera_control = AP::ircamera_control();
        if (ircamera_control == nullptr) {
               //gcs().send_text(MAV_SEVERITY_CRITICAL, "Inside IR");
                    return;
        }
        //gcs().send_text(MAV_SEVERITY_CRITICAL, "IROut");
        ircamera_control->send_camera_settings_status(chan);
    }
    */
}
...
MAV_RESULT GCS_MAVLINK::handle_command_long_packet(const mavlink_command_long_t &packet)
{
    ...
    case MAV_CMD_DO_SET_ROI:
        gcs().send_text(MAV_SEVERITY_INFO,"ROI recieved");
...
MAV_RESULT GCS_MAVLINK::handle_command_do_set_roi(const Location &roi_loc)
{
    AP_Mount *mount = AP::mount();
    if (mount == nullptr) {
        gcs().send_text(MAV_SEVERITY_INFO, "setting ROI-1");
        return MAV_RESULT_UNSUPPORTED;
    }

    // sanity check location
    if (!roi_loc.check_latlng()) {
        gcs().send_text(MAV_SEVERITY_INFO, "setting ROI-2");
        return MAV_RESULT_FAILED;
    }
    ...
    if (roi_loc.lat == 0 && roi_loc.lng == 0 && roi_loc.alt == 0) {
        ...
        gcs().send_text(MAV_SEVERITY_INFO, "setting ROI-5");
    } else {
        // send the command to the camera mount
        mount->set_roi_target(roi_loc);
        gcs().send_text(MAV_SEVERITY_INFO, "setting ROI-4");
    }
    return MAV_RESULT_ACCEPTED;
}
...
MAV_RESULT GCS_MAVLINK::handle_command_int_packet(const mavlink_command_int_t &packet)
{
    ...
    case MAV_CMD_DO_SET_ROI:
    case MAV_CMD_DO_SET_ROI_LOCATION:
        gcs().send_text(MAV_SEVERITY_INFO, "ROI INT CMD");
