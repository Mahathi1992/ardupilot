...
void GCS_MAVLINK_Plane::handle_power12v_set(const mavlink_message_t *msg){
    mavlink_power_12v_t packet;
    mavlink_msg_power_12v_decode(msg, &packet);
    mavlink_msg_power_12v_send(chan,packet.Power12_On_Off,packet.Roll,packet.Pitch,packet.Yaw, packet.Mount_Mode);
#if MOUNT == ENABLED
    /*
            if(!plane.camera_mount.has_pan_control()) {
                plane.control_mode->auto_yaw.set_fixed_yaw(
                    (float)packet.Yaw / 100.0f,
                    0.0f,
                    0,0);
            }
            */
            //gcs().send_text(MAV_SEVERITY_NOTICE, "tilt %f", packet.Pitch);
            //plane.camera_mount.control(packet.Pitch, packet.Roll, packet.Yaw, (MAV_MOUNT_MODE) packet.Mount_Mode);

#endif
    // gcs().send_text(MAV_SEVERITY_CRITICAL, "Power Status %d", packet.Power12_On_Off);
}
